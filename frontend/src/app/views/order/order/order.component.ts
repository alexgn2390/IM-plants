      ...
      this.userService.getUserInfo()
        .subscribe((data: UserInfoType | DefaultResponseType) => {
          ...
          const paramsToUpdate = {
            ...
            comment: userInfo.comment ? userInfo.comment : ''
          }
    ...
    if (this.orderForm.valid && this.orderForm.value.firstName && this.orderForm.value.lastName
      ...
      if (this.deliveryType === DeliveryType.delivery) {
        ...
        if (this.orderForm.value.house) {
          paramsObject.street = this.orderForm.value.house
        }
        if (this.orderForm.value.entrance) {
          paramsObject.street = this.orderForm.value.entrance
        }
        if (this.orderForm.value.apartment) {
          paramsObject.street = this.orderForm.value.apartment
        }
        if (this.orderForm.value.comment) {
          paramsObject.street = this.orderForm.value.comment
        }
      }
